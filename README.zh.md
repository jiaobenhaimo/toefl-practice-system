...
4. **阅读：** 允许回到上一题，其他部分只能向前。
5. **听力：** 音频自动播放一次，播放期间"下一题"按钮和选项均被禁用（灰色不可点击）。不能重播。
6. **口语：** 全自动流程。提示音频播放完毕后出现 3 秒倒计时（"Recording in 3... 2... 1..."），随后麦克风自动开启并显示实时声波图。问题计时结束后自动停止录音并跳转到下一题。
7. **写作：** 在文本框中输入，下方实时显示字数。
8. 不同部分之间的过渡页面显示"部分完成"和下一部分信息，不显示分数。所有分数在最终成绩页面统一展示。
...
toefl-practice-system/
  ...
  tests/                    试题内容（*.md、*.tts、音频文件夹）
    example-test.md         示例试卷（包含所有 7 种题型）
```
